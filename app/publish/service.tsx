...
import { ArrowLeft, Check, ChevronRight, ChevronLeft, ChevronDown, ImageIcon, X, User, Heart } from 'lucide-react-native';
...
export default function PublishServiceScreen() {
  ...
  const handleSubmit = async () => {
    ...
    showAlert(
      ...
      isContribution
        ? 'Cuando el dueño se registre, le avisaremos que puede reclamar este servicio.'
        : 'Tu servicio ya está visible para todos.'
    );
    router.back();
  };
    ...
    <View style={styles.stepContent}>
      ...
      {isContribution && (
        <View style={{
          ...
          <Text style={{ fontSize: 13, color: tc.textSecondary, lineHeight: 18 }}>
            Estás publicando el servicio de otra persona. Si esa persona se registra, le avisaremos que puede reclamar este listado como suyo.
          </Text>
        </View>
      )}
      ...
      {isContribution && (
        <View style={{
          ...
          <TextInput
            style={[styles.input, { color: tc.text, backgroundColor: tc.bgInput, borderColor: tc.borderLight }, Platform.OS === 'web' ? ({ outlineStyle: 'none' } as any) : null]}
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: tc.bg }]}>
      ...
      >
        ...
        <View style={[styles.header, { borderBottomColor: tc.borderLight }]}>
          ...
          <Text style={[styles.headerTitle, { color: tc.text }]}>{isContribution ? (isEditing ? 'Editar contribución' : 'Contribuir con Un Pique') : (isEditing ? 'Editar servicio' : 'Publicar servicio')}</Text>
